...
class InterpolatedCurve(maxDeg: Int): Bspline(maxDeg) {

    val pts = mutableListOf<Vector3>()

    fun addPts(v: Vector3) {
        pts.add(v); ctrlPts.add(v)
        degree(); order(); evalPrm(); evalKnots(); evalCtrlPoints()
    }
    ...
    fun removePts(i: Int) {
        ...
        if(!pts.isEmpty()) degree(); order(); evalPrm(); evalKnots(); evalCtrlPoints()
    }
